  ...
  if (sl_string__scanf(command_params, "0x%x 0x%x %u", &slave_address, &slave_register, &count) >= 2) {
    if (i2c__read_slave_data(i2c_bus, slave_address, slave_register, &buffer[0], count)) {
      ...
      for (size_t index = 0; index < count; index++) {
        // Changed this for loopback print [for i2c lab]
        sl_string__printf(output, "  0x%02X: 0x%02X (%d)\n", ((slave_register + index) % 256), buffer[index],
                          buffer[index]);
        cli_output(unused_cli_argument, sl_string__c_str(output));
      }
    } else {
...
static void cli__i2c_write(sl_string_s command_params, app_cli__print_string_function cli_output) {
  ...
  if (2 == sl_string__scanf(command_params, "0x%x 0x%x", &slave_address, &slave_register)) {
    ...
    if (i2c__write_slave_data(i2c_bus, slave_address, slave_register, &buffer[0], count)) {
      ...
      for (size_t index = 0; index < count; index++) {
        // Changed this for loopback print [for i2c lab]
        sl_string__printf(output, "  [%3d] [0x%02X] = 0x%02X (%d)\n", ((slave_register + index) % 256),
                          ((slave_register + index) % 256), buffer[index], buffer[index]);
        cli_output(unused_cli_argument, sl_string__c_str(output));
      }
    } else {
